...
import BookingModificationAudit from './BookingModificationAudit.js';
...
export default function AdminDashboard({
  ...
  const [logsSubTab, setLogsSubTab] = useState<'modification_audit' | 'all_logs' | 'backup_control'>('modification_audit');
          ...
          {activeTab === 'logs' && (() => {
            ...
            return (
              <div className="space-y-6 animate-fade-in text-xs font-sans">
                ...
                <div className="flex flex-wrap border-b border-slate-800 pb-px gap-2">
                  {[
                    { id: 'modification_audit', label: '🛡️ Booking Audit Log', desc: 'Critical booking modifications' },
                    { id: 'all_logs', label: '🌐 All Operational Logs', desc: 'Full administrative stream' },
                    { id: 'backup_control', label: '💾 Disaster Recovery Core', desc: 'Secure backups & snapshots' }
                  ].map((sub) => (
                    <button
                      key={sub.id}
                      type="button"
                      onClick={() => setLogsSubTab(sub.id as any)}
                      className={`py-2 px-4 border-b-2 text-xs font-bold transition-all cursor-pointer mr-1 rounded-t-xl text-left ${
                        logsSubTab === sub.id
                          ? 'border-emerald-500 text-white bg-emerald-500/5 font-black'
                          : 'border-transparent text-slate-400 hover:text-slate-200 hover:bg-slate-900/30'
                      }`}
                    >
                      <p className="text-xs">{sub.label}</p>
                      <p className={`text-[9px] font-medium hidden sm:block ${logsSubTab === sub.id ? 'text-emerald-400/80' : 'text-slate-500'}`}>{sub.desc}</p>
                    </button>
                  ))}
                </div>

                {logsSubTab === 'modification_audit' && (
                  <BookingModificationAudit 
                ...
                {logsSubTab === 'backup_control' && (
                  <div className="space-y-6 animate-fade-in">
                    {/* Disaster Recovery Backups Control Panel */}
                    <div className="bg-slate-900/30 border border-slate-800 p-6 rounded-2xl space-y-6 font-sans">
